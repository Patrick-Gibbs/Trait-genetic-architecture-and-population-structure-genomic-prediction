...
for trait in ['study_4_M130T666']:#tqdm(['dummy', 'flowering_time_10deg', 'herbavore_resistance_G2P', 'Metabolite146']):
    trait_results = []
    for pc_var in tqdm(PC_VARIANCES_TO_TEST):
        ...
        trait_results += test_ridge_model(X, y, cv=cv, alphas= [2**x for x in range(-20,20)], n_jobs=-1, pca_variance=pc_var, name_of_feature_representations='PCA_SNPS'
                                          , ridge_lasso_enet=[True, False, False], )
        

        trait_results += test_ridge_model(X, y, cv=cv, alphas= [1.5**x for x in range(-10,10)], n_jobs=-1, pca_variance=pc_var, name_of_feature_representations='PCA_SNPS',
                                           ridge_lasso_enet=[False, True, False], lasso_tol=0.01, lasso_cv=my_Kfold(k=5,state=2))
        
        
        trait_results += test_ridge_model(X, y, cv=cv, alphas= [2**x for x in range(-10,10)], n_jobs=-1, pca_variance=pc_var, name_of_feature_representations='PCA_SNPS',
